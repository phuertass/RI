...
public class IndiceSimple {
	...
	private static final String SAMPLE_CSV_FILE_PATH = "./n_movies.csv";
	...
	public void indexarDocumentos() throws IOException{
		...
		for(CSVRecord csvRecord : csvParser){
			...
			fconfig.setMultiValued("certificate", true);
			fconfig.setMultiValued("Duration", true);
			fconfig.setMultiValued("Genre", true);
			fconfig.setMultiValued("Rating", true);
			fconfig.setMultiValued("Description", true);
			fconfig.setMultiValued("Stars", true);
			fconfig.setMultiValued("Votes", true);

			if(!csvRecord.get(0).isEmpty()){
				doc.add(new TextField(campos[0], csvRecord.get(0), Field.Store.YES)); //titulo
				doc.add(new FacetField(campos[0], csvRecord.get(0))); //TITLE
			}
			...
			if(!csvRecord.get(2).isEmpty()){
				...
				doc.add(new FacetField(campos[2], getCertificate(csvRecord.get(2)))); //CERTIFICATE
			}
			...
			if(!csvRecord.get(3).isEmpty()){
				...
				doc.add(new FacetField(campos[3], getCertificate(csvRecord.get(3)))); //CERTIFICATE
			}

			if(!csvRecord.get(4).isEmpty()){
				doc.add(new TextField(campos[4], getGenre(csvRecord.get(4)), Field.Store.YES)); //genre
				doc.add(new FacetField(campos[4], getCertificate(csvRecord.get(4)))); //CERTIFICATE
			}
			...
			if(!csvRecord.get(5).isEmpty()){
				...
				doc.add(new FacetField(campos[5], getCertificate(csvRecord.get(5)))); //CERTIFICATE
			}
			
			if(!csvRecord.get(6).isEmpty()){
				doc.add(new TextField(campos[6], csvRecord.get(6), Field.Store.YES)); //description 
				doc.add(new FacetField(campos[6], getCertificate(csvRecord.get(6)))); //CERTIFICATE
			}

			if(!csvRecord.get(7).isEmpty()){
				doc.add(new TextField(campos[7], getStars(csvRecord.get(7)), Field.Store.YES)); //stars
				doc.add(new FacetField(campos[7], getCertificate(csvRecord.get(7)))); //CERTIFICATE
			}
			...
			if(!csvRecord.get(8).isEmpty()){
				...
				doc.add(new FacetField(campos[8], getCertificate(csvRecord.get(8)))); //CERTIFICATE
			}
