...
public  void indexSearch(Analyzer analyzer, Similarity similarity){
            ...
            switch(valor){
                ...
                case 0:
                    ...
                    switch(valor){
                        ...
                        case 1:
                            System.out.println("**********************************************************\n");
                            System.out.println("Introduzca el tipo de consulta booleana que desea realizar:\n");
                            System.out.println("(0) Consulta Booleana por campo TODO\n(1) Consulta Booleana con campos por defecto\n(2) Consulta Booleana por los campos que quieras\n");
                            sc = new Scanner(System.in);
                            ...
                            switch(valor2){
                                case 0:
                                    ...
                                    break;
                                case 1: 
                                    documentos = Booleana2Campos(analyzer);
                                    break;
                                case 2: 
                                    System.out.println("**********************************************************\n");
                                    System.out.println("Introduzca los campos por los que desea buscar separados por espacios en blanco (ejemplo: genre year title): ");
                                    ...
                                    break;
                                default:
                                    System.out.println("Error para booleanas");
                                    break;
                            }
    ...
    public TopDocs ConsultaFrase(String campo, Analyzer analyzer) throws IOException{
        ...
        //separamos la consulta por palabras 
    ...
    public TopDocs ConsultaMultiple(Analyzer analyzer, String[] campos) throws IOException{
        ...
        //parseamos el num de campo
        QueryParser[] parser = new QueryParser[numcampos];
        String[] values = new String[numcampos];

        //recorremos campos
        for(int i=0; i<numcampos; i++){
            parser[i] = new QueryParser(campos[i], analyzer);
            System.out.println("Introduzca el " + campos[i] + ": "); //introducimos el token en el campo correspondiente
            Scanner sc = new Scanner(System.in);
            values[i] = sc.nextLine();
        }

        //hacemos consulta booleana
        BooleanQuery.Builder bqbuilder = new BooleanQuery.Builder();
        Query[] q = new Query[numcampos];

        try{
            for(int i=0; i<numcampos; i++){
                q[i]=parser[i].parse(values[i]); //parseamos la query con los campos
            }
        }catch (org.apache.lucene.queryparser.classic.ParseException e){
            System.out.println("¡¡¡Error en la consulta multiple!!!");
        }


        BooleanClause[] bc = new BooleanClause[numcampos];
        for(int i=0; i<numcampos; i++){
    ...
    public TopDocs Booleana2Campos(Analyzer analyzer) throws IOException{
        QueryParser titulo = new QueryParser("title", analyzer);
        QueryParser duration = new QueryParser("duration", analyzer);

        System.out.println("Introduzca el titulo: ");
        Scanner sc1 = new Scanner(System.in);
        String tit = sc1.nextLine();

        System.out.println("Introduzca la duracion: ");
        Scanner sc2 = new Scanner(System.in);
        String dur = sc2.nextLine();
        ...
        try{
            q1 = titulo.parse(tit);
            q2 = duration.parse(dur);
        }catch (org.apache.lucene.queryparser.classic.ParseException e){
            System.out.println("¡¡¡Error en la consulta booleana por dos terminos!!!");
        }

        BooleanClause bc1 = new BooleanClause(q1, BooleanClause.Occur.MUST);
        BooleanClause bc2 = new BooleanClause(q2, BooleanClause.Occur.MUST);

        //añadimos a un objeto de la clase BooleanQuery.Builder para construir la query final
        bqbuilder.add(bc1);
        bqbuilder.add(bc2);
