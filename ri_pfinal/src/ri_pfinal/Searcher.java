...
DrillDownQuery ddq;
    ...
    Searcher(){
        capsPath="src/indicecaps";
        
        facetasPath="src/facet";
        
        try{
        //Obtenemos directorios
        //cambiar segun el directorio que queremo
        //indexPath= FSDirectory.open(Paths.get(dir));
        Directory taxoDir = FSDirectory.open(Paths.get(facetasPath));
        //Abrimos directorios
...
public String indexSearch(String consulta, String type,String[] campos,Directory dir){
    Analyzer analyzer = new StandardAnalyzer();
    Similarity similarity = new ClassicSimilarity();
       ...
       //DIRECTORIO DE LOS INDICES
        ...
        switch(type){
            case "Consulta Generica":
                documentos = ConsultaGenerica(campos[0], analyzer, consulta);
                break;
            case "Consulta Booleana":
                documentos = ConsultaBooleana(campos[0], analyzer, consulta);
                break;
            case "Consulta Frases":
                documentos = ConsultaFrase(campos[0], analyzer, consulta);
                break;
...
public  TopDocs ConsultaGenerica(String campo, Analyzer analyzer, String consulta) throws IOException {
    ...
    public TopDocs ConsultaMultiple(Analyzer analyzer, String[] campos, String consulta) throws IOException{
